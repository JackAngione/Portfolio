...
if server_type == "BACKEND":
    ...
    DOCKERFILE_PATH = (
        "./backend_bun"  # Path to the DIRECTORY containing your Dockerfile.
    )
    ...
    DOCKERFILE_PATH = (
        "./mediaserver"  # Path to the DIRECTORY containing your Dockerfile.
    )
...
def run_command(command, cwd=None):
    ...
    try:
        ...
        print(
            f"Error: The command '{command[0]}' was not found. Make sure Docker is installed and in your PATH."
        )
        raise
...
if __name__ == "__main__":
    ...
    try:
        login = run_command(
            login_command_args
        )  # docker run in detached mode outputs the container ID
    ...
    if push_to_GitLab:
        ...
        run_command_args = ["docker", "push", full_image_name]
        try:
            push_id = run_command(
                run_command_args
            )  # docker run in detached mode outputs the container ID
    ...
    try:
        ...
        print(
            f"Could not stop/remove existing container (it might not exist, or Docker is not running): {e}"
        )
