...
const ArtistDisplay = ({artist_list, sendSelectedSong}) => {
    ...
    const fetchSongs = async (artist_id) => {
        try {
            ...
            const response = await fetch(`http://192.168.1.204:808/artist/${artist_id}`);
            ...
            {artist_list.map((item, index) => (
                <div key={index} className="mb-2 rounded-lg overflow-hidden">
                    ...
                    <button
                        onClick={() => {
                            toggleItem(index)
                            fetchSongs(item.artist_id).then(r => {setSongList(r)})
                        }}
                    ...
                    >
                        <div className="px-4 py-3 text-black">
                            {songList.map((song, index) => (
                                ...
                                <button className="outline-black " key={index} onClick={() => handleSongClick(song.artist_id, song.song_id)}
                                >
                                    <LazyLoadImage
